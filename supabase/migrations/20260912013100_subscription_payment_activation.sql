...
create table public.subscription_payment_intents (
  ...
  gateway text not null check (gateway in ('DEMO','CMI','PAYPAL')),
  provider_intent_id text not null check (length(provider_intent_id) between 8 and 200),
  ...
  unique (gateway, provider_intent_id),
  unique (id, organization_id)
);

create table public.subscription_payment_events (
  id bigint generated always as identity primary key,
  payment_intent_id uuid not null references public.subscription_payment_intents(id) on delete restrict,
  ...
  activation_result jsonb not null check (jsonb_typeof(activation_result) = 'object'),
  correlation_id uuid not null,
  received_at timestamptz not null default clock_timestamp(),
  unique (gateway, provider_event_id),
  foreign key (payment_intent_id, organization_id)
    references public.subscription_payment_intents(id, organization_id) on delete restrict
);
...
revoke all on public.subscription_payment_intents, public.subscription_payment_events
  from public, anon, authenticated, service_role;

create trigger subscription_payment_intents_immutable
before update or delete on public.subscription_payment_intents
...
create function public.register_subscription_payment_intent(
...
begin
  ...
  if p_amount_minor <> expected_amount or p_currency <> plan.currency then
...
create function public.process_verified_subscription_payment(
  p_gateway text,
...
begin
  ...
  if p_gateway <> 'DEMO' or p_event_type <> 'PAYMENT_SUCCEEDED'
  ...
  if intent.amount_minor <> p_amount_minor or intent.currency <> p_currency then
  ...
  if found then
    if payment_event.payment_intent_id <> intent.id
      or payment_event.payload_hash <> p_payload_hash
      or payment_event.amount_minor <> p_amount_minor
      or payment_event.currency <> p_currency then
  ...
  result := jsonb_build_object('outcome','SUBSCRIPTION_ACTIVATED','subscription_id',subscription.id,'payment_intent_id',intent.id,'provider_event_id',p_provider_event_id,'cycle_number',cycle_number,'period_end',period_end,'amount_minor',intent.amount_minor,'currency',intent.currency,'replayed',false);
  ...
  values(intent.organization_id,'SERVICE','subscription.activated_from_verified_payment','subscription',subscription.id::text,p_correlation_id,jsonb_build_object('payment_intent_id',intent.id,'provider_event_id',p_provider_event_id,'cycle_number',cycle_number),repeat('0',64));
  insert into public.event_outbox(organization_id,aggregate_type,aggregate_id,event_type,correlation_id,payload,idempotency_key)
  values(intent.organization_id,'subscription',subscription.id::text,'SubscriptionPaidV1',p_correlation_id,result,p_provider_event_id);
  return result;
end
$$;
...
create index subscription_payment_intents_org_created_idx on public.subscription_payment_intents(organization_id,created_at desc);
create index subscription_payment_events_intent_received_idx on public.subscription_payment_events(payment_intent_id,received_at desc);
