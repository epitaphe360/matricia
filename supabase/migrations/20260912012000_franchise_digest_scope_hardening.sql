-- P1 MAT-FUNC-064: exact franchise scope in multi-franchise operator organizations.

create or replace function private.franchise_digest_role_access(p_franchise_id uuid,p_allow_platform_admin boolean default true,p_actor uuid default auth.uid())returns boolean language sql stable security definer set search_path=pg_catalog,public,private as $$
  ...
  where f.id=p_franchise_id and f.status='ACTIVE'and(
   ...
   or exists(
    ...
    where m.organization_id=f.operator_organization_id and m.user_id=p_actor and m.status='ACTIVE'
     ...
     and(
      mr.franchise_id=f.id
      or(mr.franchise_id is null and(select count(*)from public.franchises sibling where sibling.operator_organization_id=f.operator_organization_id and sibling.status='ACTIVE')=1)
     )
   )
  )
 )
...
create function private.guard_franchise_digest_config_scope()returns trigger language plpgsql security definer set search_path=pg_catalog,public,private as $$
begin
 if not private.franchise_digest_role_access(new.franchise_id,true,new.created_by)then raise exception'FRANCHISE_DIGEST_CONFIG_ACTOR_SCOPE_DENIED'using errcode='42501';end if;
 if not private.franchise_digest_role_access(new.franchise_id,false,new.recipient_user_id)then raise exception'FRANCHISE_DIGEST_CONFIG_RECIPIENT_SCOPE_DENIED'using errcode='42501';end if;
 return new;
end$$;
create trigger franchise_digest_config_scope_guard before insert on public.franchise_daily_digest_config_versions for each row execute function private.guard_franchise_digest_config_scope();
...
revoke all on function private.franchise_digest_role_access(uuid,boolean,uuid),private.guard_franchise_digest_config_scope()from public,anon,authenticated,service_role;
