-- Franchisees submit SERVICE drafts to Matricia via catalog_change_requests.
-- decide_catalog_change already allowed SERVICE on the table check; this adds
-- the missing command path and the approval/rejection state machine.

create function public.submit_franchise_catalog_service(
  p_service_id uuid,
  p_version_id uuid,
...
revoke all on function public.submit_franchise_catalog_service(uuid, uuid, integer, integer, text, uuid)
  from public, anon, authenticated, service_role;
grant execute on function public.submit_franchise_catalog_service(uuid, uuid, integer, integer, text, uuid)
  to authenticated;
