...
select ok(not has_function_privilege('anon','public.start_questionnaire_session(uuid,uuid,text,timestamptz,text,uuid,uuid)','EXECUTE'),'anon cannot execute session start');
