...
select ok((select p.prosrc like '%iterations<24%'and p.prosrc like '%max(occurrence.scheduled_on)%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public'and p.proname='schedule_recurring_service_requests'),'scheduler bounds each plan and resumes backlog after its latest occurrence');
