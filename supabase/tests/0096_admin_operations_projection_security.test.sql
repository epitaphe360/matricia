...
select throws_ok($$set local role authenticated;select set_config('request.jwt.claim.sub','a9600000-0000-4000-8000-000000000001',true);select public.list_admin_operations_projection(10)$$,'42501','ADMIN_OPERATIONS_PROJECTION_DENIED','tenant organization role is denied');
select throws_ok($$set local role authenticated;select set_config('request.jwt.claim.sub','a9600000-0000-4000-8000-000000000002',true);select public.list_admin_operations_projection(10)$$,'42501','ADMIN_OPERATIONS_PROJECTION_DENIED','unlisted platform role is denied');
...
set local role authenticated;select set_config('request.jwt.claim.sub','a9600000-0000-4000-8000-000000000003',true);insert into p96_observed values('SUPER_ADMIN',public.list_admin_operations_projection(1));reset role;
set local role authenticated;select set_config('request.jwt.claim.sub','a9600000-0000-4000-8000-000000000004',true);insert into p96_observed values('MATRICIA_ADMIN',public.list_admin_operations_projection(1));reset role;
set local role authenticated;select set_config('request.jwt.claim.sub','a9600000-0000-4000-8000-000000000005',true);insert into p96_observed values('READ_ONLY_AUDITOR',public.list_admin_operations_projection(1));reset role;
...
select throws_ok($$set local role authenticated;select set_config('request.jwt.claim.sub','a9600000-0000-4000-8000-000000000003',true);select public.list_admin_operations_projection(0)$$,'22023','INVALID_ADMIN_OPERATIONS_LIMIT','zero limit is denied');
select throws_ok($$set local role authenticated;select set_config('request.jwt.claim.sub','a9600000-0000-4000-8000-000000000003',true);select public.list_admin_operations_projection(201)$$,'22023','INVALID_ADMIN_OPERATIONS_LIMIT','limit above 200 is denied');
