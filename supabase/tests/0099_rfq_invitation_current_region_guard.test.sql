...
select ok((select p.prosrc like '%region_code%'and p.prosrc like '%provider_service_eligibility_snapshot%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='private'and p.proname='enforce_rfq_invitation_current_eligibility'),'guard revalidates region and delegates qualification/capacity to versioned evidence');
