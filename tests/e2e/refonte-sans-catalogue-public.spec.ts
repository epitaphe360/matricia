...
for (const locale of ["fr", "ar"] as const) {
  test("parcours public " + locale + ": diagnostic, bilan, besoin et URL historique", async ({ page }, testInfo) => {
    ...
    await page.locator(".journey-textarea").first().fill(locale === "fr" ? "Sécuriser le Wi-Fi de notre bureau de quinze personnes." : "تأمين شبكة الواي فاي في مكتبنا.");
    await page.getByRole("button", { name: locale === "fr" ? "Continuer" : "متابعة" }).click();
    await page.getByRole("button", { name: locale === "fr" ? "Continuer" : "متابعة" }).click();
    await page.getByRole("button", { name: locale === "fr" ? "Continuer" : "متابعة" }).click();
    await page.getByRole("button", { name: locale === "fr" ? "Continuer" : "متابعة" }).click();
    ...
    await expect(page).toHaveURL(new RegExp("/" + locale + "/besoin\\?q=wifi"));
