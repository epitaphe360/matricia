...
export async function loadClientPortfolio(): Promise<RepoResult<Portfolio>> {
  ...
  if (organizationIds.length === 0) return { status: "success", value: { organizations: [], sites: [], projects: [], tasks: [], budgets: [], costCenters: [], allocations: [], calendar: [], libraries: [] } };
  const [roles, organizations, sites, siteVersions, projects, projectVersions, tasks, taskVersions, budgets, budgetVersions, costCenters, costCenterVersions, allocations, calendar, libraries] = await Promise.all([
    ...
    client.from("client_site_versions").select("site_id,version,name_fr,name_ar").in("organization_id", organizationIds).limit(1000),
    ...
    client.from("client_project_versions").select("project_id,version,name_fr,name_ar,description_fr,description_ar").in("organization_id", organizationIds).limit(2000),
    ...
    client.from("client_project_task_versions").select("task_id,version,title_fr,title_ar").in("organization_id", organizationIds).limit(4000),
    ...
    client.from("client_annual_budget_versions").select("budget_id,version,amount_minor,approved_amount_minor").in("organization_id", organizationIds).limit(2000),
    ...
    client.from("client_cost_center_versions").select("cost_center_id,version,name_fr,name_ar").in("organization_id", organizationIds).limit(2000),
    ...
    client.from("client_central_calendar").select("organization_id,project_id,item_id,source_kind,event_type,title_fr,title_ar,starts_at,ends_at,status").in("organization_id", organizationIds).order("starts_at", { ascending: true }).limit(2000),
    client.from("catalog_libraries").select("id,name_fr,name_ar").eq("status", "ACTIVE").limit(20)
  ]);
  const all = [roles, organizations, sites, siteVersions, projects, projectVersions, tasks, taskVersions, budgets, budgetVersions, costCenters, costCenterVersions, allocations, calendar, libraries];
