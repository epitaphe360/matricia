...
export async function loadFranchiseFollowups():Promise<FollowupLoadResult>{
 ...
 const [franchises,prospects,policies,preferences,jobs,memberships,platform]=await Promise.all([
 ...
 if([franchises,prospects,policies,preferences,jobs,memberships,platform].some((result)=>result.error))return{status:"error",reason:"QUERY_FAILED"};
 const fs=z.array(franchise).safeParse(franchises.data),ps=z.array(prospect).safeParse(prospects.data),rs=z.array(policy).safeParse(policies.data),prefs=z.array(preference).safeParse(preferences.data),js=z.array(job).safeParse(jobs.data),ms=z.array(membership).safeParse(memberships.data),roles=z.array(z.object({role_code:z.string()})).safeParse(platform.data);
 if(!fs.success||!ps.success||!rs.success||!prefs.success||!js.success||!ms.success||!roles.success)return{status:"error",reason:"INVALID_RESPONSE"};
 const platformRoles=new Set(roles.data.map((row)=>row.role_code)),writableOrganizations=new Set(ms.data.filter((row)=>row.organization_member_roles.some((role)=>!role.revoked_at&&["FRANCHISE_OWNER","FRANCHISE_MANAGER"].includes(role.role_code))).map((row)=>row.organization_id));
 const canApprove=platformRoles.has("SUPER_ADMIN")||platformRoles.has("MATRICIA_ADMIN"),franchiseRows=fs.data.map((row)=>({id:row.id,operatorCode:row.operator_code,canWrite:canApprove||writableOrganizations.has(row.operator_organization_id)}));
