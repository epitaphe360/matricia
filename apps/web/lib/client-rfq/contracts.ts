import type { ClientOrganization, ClientRequestSummary, QuoteComparisonRow } from "./model";
...
export type ClientRfqRepository = {
  ...
  select(input: { quoteId: string; quoteVersionId: string; idempotencyKey: string; correlationId: string }): Promise<Result<{ quoteId: string; rfqId: string }>>;
};
