...
export async function createServerClientRfqRepository(){const client=await getSupabaseServerClient();return createClientRfqRepository({
 ...
 async roles(ids){if(ids.length===0)return{data:[],error:null};return await client.from("organization_member_roles").select("membership_id,role_code,revoked_at").in("membership_id",ids).in("role_code",["CLIENT_OWNER","CLIENT_ADMIN","CLIENT_BUYER"]).limit(300);},
 ...
 async quotes(ids){if(ids.length===0)return{data:[],error:null};return await client.from("quotes").select("id,rfq_id,status").in("rfq_id",ids).limit(2000);},async rpc(name,input){return await client.rpc(name,input)}
});}
