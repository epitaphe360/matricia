...
export async function createServerDisputesRepository(requestedOrganizationId?: string) {
  ...
  return createDisputesRepository({
    ...
    async missions() { let q = c.from("missions").select("id,status").in("status", ["ACTIVE", "DELIVERY_SUBMITTED", "ACCEPTANCE_IN_PROGRESS"]); if (organizationId) q = q.eq("client_organization_id", organizationId); return q.order("created_at", { ascending: false }).limit(200); },
    async cases(id) { let q = c.from("dispute_cases").select("id,mission_id,client_organization_id,provider_organization_id,obligation_key,description,urgency,status,policy_snapshot,response_due_at,review_due_at,appeal_due_at,row_version"); if (organizationId) q = q.eq("client_organization_id", organizationId); if (id) q = q.eq("id", id); return q.order("opened_at", { ascending: false }).limit(200); },
