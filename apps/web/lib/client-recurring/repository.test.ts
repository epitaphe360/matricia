...
function source(overrides: Partial<ClientRecurringSource> = {}): ClientRecurringSource {
  return {
    user: async () => id(1),
    requests: async () => ({ data: [{ id: id(2), current_version_id: id(3), status: "CONTRACTED", created_at: "2026-09-01T00:00:00Z" }], error: null }),
    ...
    plans: async () => ({ data: [{ id: id(4), template_request_id: id(2), status: "ACTIVE", current_version: 2, row_version: 2, updated_at: "2026-09-12T00:00:00Z" }], error: null }),
...
describe("client recurring repository", () => {
  it("assemble seulement la version courante et les occurrences du plan", async () => { const result = await createClientRecurringRepository(source()).load(); expect(result).toMatchObject({ status: "success", value: { requests: [{ id: id(2), versionNumber: 4 }], plans: [{ id: id(4), currentVersion: 2, occurrences: [{ generatedRequestId: id(7) }] }] } }); });
