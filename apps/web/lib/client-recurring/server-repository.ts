...
export async function createServerClientRecurringRepository() {
  ...
  return createClientRecurringRepository({
    ...
    async requests() { return await client.from("service_requests").select("id,client_organization_id,current_version_id,status,created_at").order("created_at", { ascending: false }).limit(200); },
    ...
    async plans() { return await client.from("recurring_service_plans").select("id,client_organization_id,template_request_id,status,current_version,row_version,updated_at").order("updated_at", { ascending: false }).limit(100); },
