...
export const adminFinanceDashboardSchema = z.object({
  ...
  provider_invoices: z.array(z.object({ id: adminFinanceUuid, ...organization, invoice_number: z.string().min(3).max(80), currency, total_minor: exactMinor, paid_minor: exactMinor, outstanding_minor: exactMinor, payment_status: z.enum(["ISSUED", "PARTIALLY_PAID", "PAID", "OVERDUE"]), due_on: z.string().min(10) }).strict()).max(200),
}).strict();
