...
import type { ProviderQuoteDashboard } from "./model";
...
export async function loadProviderQuotes(requestedOrganizationId?: string): Promise<{ status: "success"; dashboard: ProviderQuoteDashboard } | Failure> {
  ...
  const invitationRows = invites.data.map((item) => {
    ...
    return { id: item.id, status: item.status, rowVersion: item.row_version, rfqId: item.rfq_id, deadline: item.rfqs.deadline, requestId: item.rfqs.request_id, description: requestVersion.description, regionCode: String(requestVersion.required_quote_data.region_code ?? "—"), currency: requestVersion.currency_code, taxCategoryCode, quote: currentQuote ? { id: currentQuote.id, status: currentQuote.status, currentVersionId: currentQuote.current_version_id, versionNumber: currentQuoteVersion?.version_number ?? null, currency: currentQuoteVersion?.currency ?? null, subtotalMinor: currentQuoteVersion?.subtotal_minor ?? null, taxMinor: currentQuoteVersion?.tax_minor ?? null, totalMinor: currentQuoteVersion?.total_minor ?? null } : null };
  });
