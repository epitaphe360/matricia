...
export function PublicNavigation({ locale }: { locale: Locale }) {
  ...
  return <header dir={locale === "ar" ? "rtl" : "ltr"} className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/85">
    <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:min-h-18">
      <Link href={`/${locale}`} aria-label={copy.brandLabel} className="flex items-center gap-2 rounded-md font-semibold tracking-tight text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
        <span aria-hidden="true" className="grid size-9 place-items-center rounded-xl bg-primary text-lg font-black text-white shadow-sm">M</span>
      ...
      <nav aria-label={copy.navigationLabel} className="hidden items-center gap-1 sm:flex">
        ...
        <Link href={`/${locale}/services`} aria-label={copy.services} className="grid size-11 place-items-center rounded-md text-foreground/70 hover:bg-accent"><Search aria-hidden="true" className="size-4" /></Link>
        ...
        <Link href={`/${locale}/connexion`} className={cn(buttonVariants(), "ms-1 min-h-11 rounded-xl px-5")}>{copy.signIn}</Link>
      </nav>
