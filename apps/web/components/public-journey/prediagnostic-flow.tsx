...
const optionIds: Record<Exclude<QuestionId, "next_action">, readonly string[]> = {
  ...
  sector: ["professional_services", "commerce", "construction", "industry_logistics", "other"],
...
function convertLegacy(raw: string, locale: Locale): Stored | null {
  try {
    ...
    for (const [rawIndex, rawAnswer] of Object.entries(value.answers)) {
      const index = Number(rawIndex), id = questionIds[index];
      ...
      if (id === "next_action") answers[id] = rawAnswer;
        ...
        const labels = source[index]?.[2] ?? [], ids = optionIds[id];
...
export function PrediagnosticFlow({ locale, organizations = [], authenticated = false, saveAction }: { locale: Locale; organizations?: OrganizationOption[]; authenticated?: boolean; saveAction?: (state: SaveState, data: FormData) => Promise<SaveState> }) {
  ...
  if (state.step >= questionIds.length) return <Result locale={locale} answers={state.answers} storageAvailable={storageAvailable} organizations={organizations} authenticated={authenticated} saveAction={saveAction} onSaved={clearStored} onRestart={() => setState(initial())} onErase={() => {
    clearStored();
    setState(initial());
  }} />;
  ...
  return <main id="contenu-principal" className="journey-shell" dir={locale === "ar" ? "rtl" : "ltr"}><div className="journey-card">
    <p className="journey-eyebrow">{copy.eyebrow}</p><p className="journey-progress" aria-live="polite">{copy.progress(state.step + 1, questionIds.length)}</p><div className="journey-progressbar" aria-hidden="true"><span style={{ width: ((state.step + 1) / questionIds.length) * 100 + "%" }} /></div>
    ...
    {choices.length ? <Choices label={String(question[0])} choices={choices} value={answer} multi={questionId === "goals"} onChange={setAnswer} /> : <textarea aria-labelledby="diagnostic-question" className="journey-textarea" autoFocus value={typeof answer === "string" ? answer : ""} onChange={(event) => setAnswer(event.target.value.slice(0, 500))} rows={5} maxLength={500} />}
    <div className="journey-actions">{state.step > 0 ? <button type="button" className="journey-secondary" onClick={() => setState((current) => ({ ...current, step: current.step - 1 }))}><ArrowLeft aria-hidden="true" size={18}/>{copy.back}</button> : <Link className="journey-secondary" href={`/${locale}`}>{copy.back}</Link>}<button type="button" className="journey-primary" disabled={!canProceed} onClick={() => setState((current) => ({ ...current, step: current.step + 1 }))}>{state.step === questionIds.length - 1 ? copy.result : copy.next}<ArrowRight aria-hidden="true" size={18}/></button></div>
  </div></main>;
}

function Choices({ label, choices, value, multi, onChange }: { label: string; choices: readonly { id: string; label: string }[]; value: Answer | undefined; multi: boolean; onChange: (value: Answer) => void }) {
  const selected = Array.isArray(value) ? value : value ? [value] : [];
  return <div className="journey-choices" role={multi ? "group" : "radiogroup"} aria-label={label}>{choices.map((choice) => <button key={choice.id} type="button" role={multi ? undefined : "radio"} aria-pressed={multi ? selected.includes(choice.id) : undefined} aria-checked={multi ? undefined : selected.includes(choice.id)} className={selected.includes(choice.id) ? "is-selected" : ""} onClick={() => multi ? onChange(selected.includes(choice.id) ? selected.filter((item) => item !== choice.id) : [...selected, choice.id]) : onChange(choice.id)}><span>{choice.label}</span><CheckCircle2 aria-hidden="true" size={20}/></button>)}</div>;
}

function Result({ locale, answers, storageAvailable, organizations, authenticated, saveAction, onSaved, onRestart, onErase }: { locale: Locale; answers: Stored["answers"]; storageAvailable: boolean; organizations: OrganizationOption[]; authenticated: boolean; saveAction?: (state: SaveState, data: FormData) => Promise<SaveState>; onSaved: () => void; onRestart: () => void; onErase: () => void }) {
  const copy = getPublicJourneyCopy(locale).diagnostic;
  ...
  const nextPath = `/${locale}/diagnostic?resume=1`, next = `/${locale}/connexion?mode=inscription&role=client&next=${encodeURIComponent(nextPath)}`;
  ...
  return <main id="contenu-principal" className="journey-shell" dir={locale === "ar" ? "rtl" : "ltr"}><section className="journey-card journey-result"><p className="journey-eyebrow">{copy.eyebrow}</p><h1>{copy.resultTitle}</h1><p className="journey-based">{copy.basedOn}</p><div className="journey-scope"><div><strong>{copy.scope}</strong><span>{questionIds.length} {locale === "fr" ? "questions déclaratives" : "أسئلة تصريحية"}</span></div><div><strong>{copy.limits}</strong><span>{locale === "fr" ? "Ce bilan ne couvre pas les sujets non abordés ni une conformité légale." : "لا يغطي هذا التقييم المواضيع غير المطروحة ولا يشكل إثبات امتثال قانوني."}</span></div></div>
    {priorities.length ? <ol className="journey-priorities">{priorities.map((key, index) => { const item = copy.priorities[key], kind = kinds[key], Icon = kind === "risk" ? AlertTriangle : kind === "verify" ? CircleHelp : Sparkles; return <li key={key} data-kind={kind}><span>0{index + 1}</span><small className="journey-kind"><Icon aria-hidden="true" size={15}/>{copy.kindLabels[kind]}</small><h2>{item[0]}</h2><p>{item[1]}</p><strong>{item[2]}</strong></li>; })}</ol> : <p role="status" className="journey-login-note">{locale === "fr" ? "Aucune priorité particulière n’est ressortie du périmètre évalué. Ce résultat n’est pas une attestation." : "لم تظهر أولوية خاصة ضمن النطاق المقيم. هذه النتيجة ليست شهادة."}</p>}
    {!authenticated ? <><p className="journey-login-note">{copy.loginNote}</p><div className="journey-actions"><button type="button" className="journey-secondary" onClick={onRestart}>{copy.restart}</button><Link className="journey-primary" href={next}>{copy.save}<ArrowRight aria-hidden="true" size={18}/></Link></div></> : organizations.length && saveAction ? <form action={submitSave} className="journey-login-note"><input type="hidden" name="locale" value={locale}/><input type="hidden" name="answers" value={JSON.stringify(answers)}/>{organizations.length === 1 ? <input type="hidden" name="organizationId" value={organizations[0]!.id}/> : <label className="grid gap-2 text-start"><strong>{locale === "fr" ? "Entreprise concernée" : "المؤسسة المعنية"}</strong><select name="organizationId" required className="min-h-11 rounded-md border bg-white px-3 text-slate-950">{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name}</option>)}</select></label>}<p className="mt-3">{locale === "fr" ? "Ce bilan sera enregistré comme projection indicative, distincte d’un diagnostic complet." : "سيتم حفظ هذا التقييم كتقدير إرشادي منفصل عن التشخيص الكامل."}</p>{saveState.status === "error" ? <p role="alert" className="mt-3 text-red-700">{saveState.reason === "FORBIDDEN" ? locale === "fr" ? "Vous n’avez plus l’autorisation d’enregistrer pour cette entreprise." : "لم يعد لديكم إذن الحفظ لهذه المؤسسة." : locale === "fr" ? "L’enregistrement a échoué. Vos réponses restent disponibles sur cet appareil." : "تعذر الحفظ. ما زالت إجاباتكم متاحة على هذا الجهاز."}</p> : null}{saveState.status === "success" ? <p role="status" className="mt-3"><CheckCircle2 aria-hidden="true" className="inline" size={18}/> {locale === "fr" ? "Bilan enregistré dans votre espace Client." : "تم حفظ التقييم في فضاء العميل."} <Link className="font-semibold underline" href={`/${locale}/client/diagnostics`}>{locale === "fr" ? "Voir mes analyses" : "عرض تحليلاتي"}</Link></p> : <button type="submit" className="journey-primary mt-4" disabled={savePending || !completeAnswers}>{savePending ? locale === "fr" ? "Enregistrement…" : "جارٍ الحفظ…" : copy.save}<ArrowRight aria-hidden="true" size={18}/></button>}</form> : <div className="journey-login-note"><p>{locale === "fr" ? "Votre compte n’a aucune organisation Client autorisée pour cet enregistrement." : "لا يحتوي حسابكم على مؤسسة عميلة مخولة لهذا الحفظ."}</p><Link className="mt-3 inline-flex font-semibold underline" href={`/${locale}/organisation?role=client`}>{locale === "fr" ? "Configurer mon organisation" : "إعداد مؤسستي"}</Link></div>}<div className="journey-links"><Link href={`/${locale}/besoin?source=diagnostic`}>{copy.prepare}</Link><Link href={`/${locale}/besoin?source=diagnostic&priority=${encodeURIComponent(priorities[0] ?? "")}`}>{copy.assist}</Link><button type="button" onClick={onErase}><Trash2 aria-hidden="true" size={15}/>{copy.erase}</button></div>{!storageAvailable ? <p role="status" className="journey-login-note">{locale === "fr" ? "Ce bilan n’est pas sauvegardé sur cet appareil." : "هذا التقييم غير محفوظ على هذا الجهاز."}</p> : null}
  </section></main>;
}
