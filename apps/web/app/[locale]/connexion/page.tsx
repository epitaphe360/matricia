...
export default async function LoginPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ next?: string | string[]; mode?: string; role?: string }> }) {
  ...
  const effectiveNextPath = registration ? `/${locale}/organisation${registrationRole === "fournisseur" ? "?role=fournisseur" : ""}` : nextPath;
  const translatedNextPath = effectiveNextPath ? `/${alternate}/${effectiveNextPath.split("/").slice(2).join("/")}` : undefined;
  const languageQuery = registration ? `?mode=inscription&role=${registrationRole}` : translatedNextPath ? `?next=${encodeURIComponent(translatedNextPath)}` : "";
    ...
    <div className="relative mx-auto w-full max-w-6xl">
      ...
      <div className="grid overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_24px_64px_rgba(7,30,53,.12)] lg:grid-cols-[.9fr_1.1fr]">
        ...
        <section className="order-1 p-6 sm:p-10 lg:order-2 lg:p-12" aria-labelledby="login-title">
          ...
          {registration ? <div className="mt-6 grid grid-cols-2 gap-2 rounded-xl bg-slate-100 p-1" aria-label={chrome.signupTitle}><Link aria-current={registrationRole === "client" ? "page" : undefined} href={`/${locale}/connexion?mode=inscription&role=client`} className="min-h-11 rounded-lg px-3 py-3 text-center text-sm font-semibold aria-[current=page]:bg-white aria-[current=page]:text-blue-700 aria-[current=page]:shadow-sm">{chrome.clientAccount}</Link><Link aria-current={registrationRole === "fournisseur" ? "page" : undefined} href={`/${locale}/connexion?mode=inscription&role=fournisseur`} className="min-h-11 rounded-lg px-3 py-3 text-center text-sm font-semibold aria-[current=page]:bg-white aria-[current=page]:text-blue-700 aria-[current=page]:shadow-sm">{chrome.providerAccount}</Link></div> : null}
