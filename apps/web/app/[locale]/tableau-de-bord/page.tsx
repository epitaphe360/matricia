...
export default async function DashboardPage({
  ...
  return <main className="min-h-dvh bg-muted/40 px-4 py-8 sm:px-6"><div className="mx-auto max-w-5xl space-y-6">{query.signout === "failed" ? <p role="alert" className="rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">{messages.signOutError}</p> : null}<header className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">{messages.eyebrow}</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">{messages.title}</h1></div><div className="flex items-center gap-3"><Link href={`/${alternate}/tableau-de-bord`} hrefLang={alternate} className="rounded-md px-3 py-2 text-sm font-medium text-primary focus-visible:outline focus-visible:outline-2">{messages.language}</Link><form action={signOut}><input type="hidden" name="locale" value={locale} /><Button type="submit" variant="outline">{messages.signOut}</Button></form></div></header><div className="grid gap-5 md:grid-cols-2"><Card><CardHeader><CardTitle>{messages.session}</CardTitle><CardDescription>{messages.signedInAs}</CardDescription></CardHeader><CardContent><p className="break-all font-medium">{user.email ?? messages.unknownAccount}</p></CardContent></Card><Card><CardHeader><CardTitle>{messages.organizationTitle}</CardTitle></CardHeader><CardContent>{organizationsError ? <p role="alert" className="text-sm text-destructive">{messages.organizationError}</p> : organizations?.length ? <ul className="space-y-2">{organizations.map((organization) => <li key={organization.id} className="rounded-lg border bg-background p-3"><span className="font-medium">{organization.display_name}</span><span className="ms-2 text-xs text-muted-foreground">{messages.organizationStatus[organization.status as keyof typeof messages.organizationStatus] ?? messages.organizationStatus.unknown}</span></li>)}</ul> : <p className="text-muted-foreground">{messages.noOrganization}</p>}</CardContent></Card></div></div></main>;
}
