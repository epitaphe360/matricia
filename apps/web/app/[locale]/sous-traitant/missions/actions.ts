...
export async function submitProviderDelivery(_:DeliveryActionState,form:FormData):Promise<DeliveryActionState>{
  ...
  const parsed=deliverySubmissionSchema.safeParse({deliverableId:text(form,"deliverableId"),description:text(form,"description"),linksText:text(form,"linksText"),proofType:text(form,"proofType"),proofLocation:text(form,"proofLocation"),evidenceHash:text(form,"evidenceHash"),proofNote:text(form,"proofNote"),idempotencyKey:text(form,"idempotencyKey")});
