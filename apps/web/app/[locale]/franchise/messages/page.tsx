...
export default async function FranchiseMessagesPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ organizationId?: string; q?: string; page?: string }> }) {
  ...
  return (
    <FranchiseAppShell locale={locale} selectedQuery={space.selectedQuery} selectedOrganizationId={space.selectedOrganizationId} userEmail={space.userEmail} active="messages" title={c.messagesTitle} lead={c.messagesLead} kicker={c.scope} mandateName={mandateName}>
      {result.status === "error" ? (
