...
export default async function FranchiseGovernancePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params; if (!isLocale(locale)) notFound(); const messages = getFranchiseMessages(locale), result = await loadFranchiseDashboard(locale); if (result.status === "error" && result.reason === "UNAUTHENTICATED") redirect(`/${locale}/connexion`);
