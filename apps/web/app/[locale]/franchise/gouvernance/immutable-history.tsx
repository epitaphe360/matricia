import { formatBps, formatMinor, type FranchiseDashboard } from "@/lib/franchise-governance/model";
...
export function GovernanceHistory({ franchise, locale, m }: { franchise: Franchise; locale: Locale; m: FranchiseMessages }) {
...
export function FinanceHistory({ book, locale, m }: { book: Book; locale: Locale; m: FranchiseMessages }) {
  return <details className="mt-4 rounded-xl border p-3"><summary className="min-h-11 cursor-pointer py-2 font-semibold">{m.immutableFinanceHistory}</summary><div className="mt-3 space-y-5">
    <section><h4 className="font-medium">{m.closureVersions}</h4><ol className="mt-2 space-y-3">{book.closures.map((closure) => <li key={closure.id} className="rounded-lg border bg-background p-3 text-sm"><div className="flex flex-wrap justify-between gap-2"><strong>{m.period} <span dir="ltr">{closure.periodStart} — {closure.periodEnd}</span> · {m.version} {closure.version}</strong><span>{formatMinor(closure.profitMinor, book.currency, locale)}</span></div><p className="mt-2 text-muted-foreground">{m.ruleVersion}: <span dir="ltr">{closure.ruleVersionId}</span></p><Hash label={m.contentHash} value={closure.contentHash}/><ul className="mt-3 space-y-1">{closure.allocations.map((allocation) => <li key={allocation.id} className="flex flex-wrap justify-between gap-2"><span dir="ltr">{allocation.beneficiaryCode} · {formatBps(allocation.shareBps)}</span><strong dir="ltr">{formatMinor(allocation.amountMinor, book.currency, locale)}</strong></li>)}</ul></li>)}</ol></section>
    <section><h4 className="font-medium">{m.distributionLedger}</h4><ol className="mt-2 space-y-2">{book.ledgerEntries.map((entry) => <li key={entry.id} className="rounded-lg border bg-background p-3 text-sm"><div className="flex flex-wrap justify-between gap-2"><span dir="ltr">#{entry.id} · {entry.entryType} · {entry.beneficiaryCode}</span><strong dir="ltr">{formatMinor(entry.amountMinor, book.currency, locale)}</strong></div><p className="mt-1 break-all text-muted-foreground" dir="ltr">{entry.referenceType}: {entry.referenceId}</p>{entry.proofHash ? <Hash label={m.proof} value={entry.proofHash}/> : null}</li>)}</ol></section>
  </div></details>;
}
