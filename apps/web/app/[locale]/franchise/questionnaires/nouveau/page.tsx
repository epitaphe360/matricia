...
export default async function FranchiseNewQuestionnairePage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ organizationId?: string }> }) {
  ...
  return (
    <FranchiseAppShell locale={locale} selectedQuery={space.selectedQuery} selectedOrganizationId={space.selectedOrganizationId} userEmail={space.userEmail} active="questionnaires" title={c.createQuestionnaire} lead={c.questionnairesLead} kicker={c.scope} mandateName={result.status === "success" ? result.workspace.mandate.libraryName : null}>
      {result.status === "error" ? (
        ...
        <FranchiseQuestionnairesWorkbench locale={locale} query={space.selectedQuery} workspace={result.workspace} selectedId={null} createMode commandIdentity={{ idempotencyKey: randomUUID(), correlationId: randomUUID() }} />
      )}
    </FranchiseAppShell>
  );
}
