...
export default async function FranchiseDocumentsPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ organizationId?: string; q?: string; page?: string }> }) {
  ...
  return (
    <FranchiseAppShell locale={locale} selectedQuery={space.selectedQuery} selectedOrganizationId={space.selectedOrganizationId} userEmail={space.userEmail} active="documents" title={c.documentsTitle} lead={c.documentsLead} kicker={c.scope} mandateName={mandateName}>
      {result.status === "error" ? (
