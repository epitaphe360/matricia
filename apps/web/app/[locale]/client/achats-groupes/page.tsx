...
import { loadVolumeProcurement } from "@/lib/volume-procurement/repository";
import { getVolumeMessages } from "./messages";
import { VolumePanel } from "./volume-panel";

export default async function VolumePage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ organizationId?: string }> }) {
  const { locale } = await params, { organizationId } = await searchParams;
  if (!isLocale(locale)) notFound();
  const messages = getVolumeMessages(locale), result = await loadVolumeProcurement(organizationId);
