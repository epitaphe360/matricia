...
export default async function SubscriptionPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className="min-h-dvh bg-muted/40 px-4 py-8 sm:px-6" dir={locale === "ar" ? "rtl" : "ltr"}>
    <div className="mx-auto max-w-6xl space-y-6">
      <nav aria-label="Navigation" className="flex flex-wrap items-center justify-between gap-3"><Link href={`/${locale}/tableau-de-bord`} className={cn(buttonVariants({variant:"outline"}),"min-h-11")}>{locale==="ar"?"لوحة التحكم":"Tableau de bord"}</Link><Link href={`/${alternate}/client/abonnement`} hrefLang={alternate} className="min-h-11 rounded-md px-3 py-2 font-medium text-primary focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2">{locale==="ar"?"Français":"العربية"}</Link></nav>
