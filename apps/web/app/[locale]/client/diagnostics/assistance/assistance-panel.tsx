...
import type { AssistanceDashboard, AssistanceSuggestion } from "@/lib/assisted-intelligence/contracts";
...
function OrganizationSelect({ id, dashboard, messages }: { id: string; dashboard: AssistanceDashboard; messages: AssistanceMessages }) {
  return <Field id={id} label={messages.organization}><select id={id} name="organizationId" required className="min-h-11 w-full rounded-md border bg-background px-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{dashboard.organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name}</option>)}</select></Field>;
}
  ...
  return <div className="space-y-6">
    <section aria-labelledby="analysis-title" className="rounded-2xl border bg-card p-4 shadow-sm sm:p-6">
      ...
      {model ? <form action={analysisAction} className="mt-5 grid min-w-0 gap-4 md:grid-cols-2">
        ...
        <OrganizationSelect id="analysis-organization" dashboard={dashboard} messages={messages}/>
        ...
        <div className="md:col-span-2"><Field id="analysis-text" label={`${messages.inputText} (${messages.optional})`} hint={messages.inputTextHelp}><textarea id="analysis-text" name="inputText" maxLength={4000} rows={4} aria-describedby="analysis-text-hint" className="w-full resize-y rounded-md border bg-background px-3 py-2"/></Field></div>
        <Field id="service-version-ids" label={`${messages.serviceIds} (${messages.optional})`}><textarea id="service-version-ids" name="serviceVersionIds" rows={3} dir="ltr" spellCheck={false} className="w-full resize-y rounded-md border bg-background px-3 py-2 font-mono text-sm"/></Field>
        <Field id="question-version-ids" label={`${messages.questionIds} (${messages.optional})`}><textarea id="question-version-ids" name="questionVersionIds" rows={3} dir="ltr" spellCheck={false} className="w-full resize-y rounded-md border bg-background px-3 py-2 font-mono text-sm"/></Field>
        ...
        <Field id="profile-reassessment-id" label={`${messages.reassessmentId} (${messages.optional})`}><input id="profile-reassessment-id" name="profileReassessmentId" inputMode="text" dir="ltr" className="min-h-11 w-full rounded-md border bg-background px-3 font-mono text-sm"/></Field>
    ...
    <section aria-labelledby="similarity-title" className="rounded-2xl border bg-card p-4 shadow-sm sm:p-6">
      ...
      {model ? <form action={similarityAction} className="mt-5 grid gap-4 md:grid-cols-2"><input type="hidden" name="locale" value={locale}/><input type="hidden" name="modelVersionId" value={model.id}/><input type="hidden" name="idempotencyKey" value={keys.similarity}/><OrganizationSelect id="similarity-organization" dashboard={dashboard} messages={messages}/><Field id="anomaly-ids" label={messages.anomalyIds}><textarea id="anomaly-ids" name="anomalyIds" required rows={4} dir="ltr" spellCheck={false} className="w-full resize-y rounded-md border bg-background px-3 py-2 font-mono text-sm"/></Field><div className="grid gap-3 md:col-span-2"><Button disabled={similarityPending} className="min-h-11 w-full sm:w-auto sm:justify-self-start">{messages.compare}</Button><Feedback state={similarityState} messages={messages}/></div></form> : null}
    </section>
