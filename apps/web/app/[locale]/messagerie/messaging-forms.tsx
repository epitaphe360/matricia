...
import type { ThreadOption } from "@/lib/internal-messaging/model";
...
export function OpenThreadForm({ options, locale, m }: { options: ThreadOption[]; locale: Locale; m: MessagingMessages }) {
  const id = useId(), router = useRouter(), [state, action, pending] = useActionState(openThreadAction, idle), key = useMemo(() => crypto.randomUUID(), []);
  useEffect(() => { if (state.status === "success") router.push(`/${locale}/messagerie?fil=${state.threadId}`); }, [state, router, locale]);
