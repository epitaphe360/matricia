...
const exportRoles = ["SUPER_ADMIN", "MATRICIA_ADMIN", "COMPLIANCE_MANAGER", "SUPPORT_AGENT", "READ_ONLY_AUDITOR"] as const;
...
export async function GET(_request: Request, context: { params: Promise<{ locale: string }> }) {
  ...
  if (!hasPlatformRole(access.roles, exportRoles) || !access.requirementSatisfied) return new Response(null, { status: 403 });
