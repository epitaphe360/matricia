...
import { loadAdminGovernanceDashboard } from "@/lib/admin-governance-disputes/repository";
import { isLocale } from "@/lib/i18n/locale";
import { cn } from "@/lib/utils";
import { GovernanceDashboard } from "./governance-dashboard";
import { getAdminGovernanceMessages } from "./messages";

export default async function AdminGovernancePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const messages = getAdminGovernanceMessages(locale);
  const result = await loadAdminGovernanceDashboard(locale);
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="min-h-dvh bg-muted/40 px-4 py-6 sm:px-6 sm:py-8"><div className="mx-auto max-w-7xl space-y-7">
    <nav aria-label={messages.eyebrow} className="flex flex-wrap items-center justify-between gap-3"><Link href={`/${locale}/tableau-de-bord`} className={cn(buttonVariants({ variant: "outline" }), "min-h-11")}>{messages.back}</Link><Link href={`/${alternate}/administration/gouvernance-franchise`} hrefLang={alternate} className="min-h-11 rounded-md px-3 py-2 font-medium text-primary focus-visible:outline focus-visible:ring-2 focus-visible:ring-ring">{messages.language}</Link></nav>
