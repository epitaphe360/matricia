...
export function QuestionnaireBuilder({ locale, library, messages, commandIdentity }: { locale: Locale; library: { id: string; code: string }; messages: BuilderMessages; commandIdentity: { idempotencyKey: string; correlationId: string } }) {
  ...
  return <section aria-labelledby={`${prefix}-title`}><Card><CardHeader><CardTitle><h2 id={`${prefix}-title`}>{messages.formTitle}</h2></CardTitle><CardDescription>{messages.formDescription} <bdi dir="ltr" className="font-mono">{library.code}</bdi></CardDescription></CardHeader><CardContent>
    <form action={action} onSubmitCapture={prepare} aria-describedby={`${prefix}-status`} className="grid gap-5 lg:grid-cols-2">
      ...
      <Field id={`${prefix}-release`} label={messages.catalogReleaseId}><Input id={`${prefix}-release`} name="catalogReleaseId" required dir="ltr" className="min-h-11 font-mono" /></Field>
      ...
      <div className="space-y-3 lg:col-span-2"><Button type="submit" disabled={pending} className="min-h-11 w-full sm:w-auto">{pending?messages.creatingForm:messages.createForm}</Button><div id={`${prefix}-status`} role={error?"alert":"status"} aria-live="polite" className={error?"min-h-6 text-sm text-destructive":"min-h-6 text-sm text-primary"}>{error??(state.status==="success"?<>{messages.formCreated} <bdi dir="ltr" className="break-all font-mono">{state.questionnaireId}</bdi></>:null)}</div></div>
