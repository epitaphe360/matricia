...
export function RuleValidationPanel({ workspace, locale, messages }: { workspace: RuleValidationWorkspace; locale: "fr" | "ar"; messages: RuleValidationMessages }) {
  ...
  return <div className="space-y-6">
    ...
    {selected ? <section className="grid gap-5 xl:grid-cols-2">
      ...
      <Card><CardHeader><CardTitle>{messages.simulationTitle}</CardTitle><CardDescription>{messages.simulationDescription}</CardDescription></CardHeader><CardContent className="space-y-4">
        <form action={simulationAction} className="space-y-4"><input type="hidden" name="locale" value={locale}/><input type="hidden" name="questionnaireVersionId" value={selected.id}/>
          <div className="space-y-2"><Label htmlFor={`${prefix}-answers`}>{messages.answers}</Label><textarea id={`${prefix}-answers`} name="answers" defaultValue="{}" rows={7} maxLength={262144} spellCheck={false} dir="ltr" aria-describedby={`${prefix}-json-help`} className={`${controlClass} min-h-36 font-mono`}/></div>
          <div className="space-y-2"><Label htmlFor={`${prefix}-previous`}>{messages.previousAnswers}</Label><textarea id={`${prefix}-previous`} name="previousAnswers" defaultValue="{}" rows={5} maxLength={262144} spellCheck={false} dir="ltr" aria-describedby={`${prefix}-json-help`} className={`${controlClass} min-h-28 font-mono`}/></div>
          <p id={`${prefix}-json-help`} className="text-sm text-muted-foreground">{messages.jsonHelp}</p><Button className="w-full" disabled={simulationPending}>{simulationPending ? messages.processing : messages.simulate}</Button>
        </form>
    ...
    <Card><CardHeader><CardTitle>{messages.questionsTitle}</CardTitle></CardHeader><CardContent>{workspace.questions.length === 0 ? <p className="text-sm text-muted-foreground">{messages.emptyQuestions}</p> : <ol className="grid gap-3 lg:grid-cols-2">{workspace.questions.map((question) => <li key={question.id} className="min-w-0 rounded-lg border p-4"><div className="flex flex-wrap items-start justify-between gap-2"><span className="font-medium">{locale === "ar" ? question.labelAr : question.labelFr}</span><Badge variant={question.required ? "default" : "secondary"}>{question.required ? messages.required : messages.optional}</Badge></div><dl className="mt-3 space-y-2 text-sm"><div><dt className="text-muted-foreground">{messages.questionId}</dt><dd className={technical} dir="ltr">{question.id}</dd></div><div><dt className="text-muted-foreground">{messages.dataKey}</dt><dd className={technical} dir="ltr">{question.dataKey}</dd></div><div><dt className="text-muted-foreground">{messages.answerType}</dt><dd>{localized(messages.answerTypes, question.answerType, messages.unknown)}</dd></div></dl></li>)}</ol>}</CardContent></Card>
