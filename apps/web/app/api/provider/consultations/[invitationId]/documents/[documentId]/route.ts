...
const authorization = z.object({
  outcome: z.literal("PROVIDER_CONSULTATION_DOCUMENT_AUTHORIZED"),
  document_id: uuid,
...
export async function GET(
  _request: Request,
  context: { params: Promise<{ invitationId: string; documentId: string }> },
) {
  const params = await context.params;
  const invitationId = uuid.safeParse(params.invitationId);
  const documentId = uuid.safeParse(params.documentId);
  if (!invitationId.success || !documentId.success) return json("RFQ_DOCUMENT_INVALID", 400);

  const userClient = await getSupabaseServerClient();
  const { data: auth } = await userClient.auth.getUser();
  if (!auth.user) return json("UNAUTHENTICATED", 401);

  const authorized = await userClient.rpc("authorize_provider_consultation_document", {
    p_rfq_provider_id: invitationId.data,
    p_document_id: documentId.data,
  });
  if (authorized.error) {
    const denied = authorized.error.code === "42501" || /UNAUTHENTICATED|RFQ_DOCUMENT_DENIED/u.test(authorized.error.message);
    return json(denied ? "RFQ_DOCUMENT_DENIED" : "RFQ_DOCUMENT_UNAVAILABLE", denied ? 403 : 503);
  }
