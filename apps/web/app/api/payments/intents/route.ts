...
export async function POST(request:Request){
  ...
  try{
    ...
    return NextResponse.json({paymentIntentId,gateway:intent.gateway,status:intent.status,amountMinor:intent.amountMinor,currency:intent.currency,confirmationUrl:intent.confirmationUrl},{status:201});
  }catch(error){return NextResponse.json({error:error instanceof PaymentGatewayError?"PAYMENT_REQUEST_REJECTED":"PAYMENT_INTENT_UNAVAILABLE"},{status:422});}
}
