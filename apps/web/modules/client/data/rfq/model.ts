...
export function formatMinorExact(value: string, currency: string, locale: "fr" | "ar"): string {
  const localeTag = locale === "ar" ? "ar-MA" : "fr-MA";
  const formatter = new Intl.NumberFormat(localeTag, { style: "currency", currency });
  const digits = formatter.resolvedOptions().maximumFractionDigits ?? 2;
  const scale = BigInt(10) ** BigInt(digits);
  const amount = BigInt(value);
  const whole = amount / scale;
  const fraction = (amount % scale).toString().padStart(digits, "0");
  const integer = new Intl.NumberFormat(localeTag, { maximumFractionDigits: 0 }).format(whole);
...
export type MatchingRunSummary = {
  ...
  status: "RUNNING" | "COMPLETED" | "NO_CANDIDATE" | "FAILED";
...
export type ClientRequestSummary = {
  ...
  budgetMinor: string | null; currency: string; createdAt: string; matchingRunId: string | null; eligibleCount: number | null; matchingHistory: MatchingRunSummary[]; rfqId: string | null; rfqDeadline: string | null; quoteCount: number;
};
