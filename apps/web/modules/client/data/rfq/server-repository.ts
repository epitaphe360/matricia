...
export async function createServerClientRfqRepository(){const client=await getSupabaseServerClient();return createClientRfqRepository({
 ...
 async matchingRuns(ids){if(ids.length===0)return{data:[],error:null};return await client.from("matching_runs").select("id,request_id,policy_version,status,target_panel_size,started_at,completed_at").in("request_id",ids).order("started_at",{ascending:false}).limit(500);},
 async matchingCandidates(ids){if(ids.length===0)return{data:[],error:null};return await client.from("matching_candidates").select("matching_run_id,eligible,exclusion_reasons,score_explanation").in("matching_run_id",ids).limit(5000);},
