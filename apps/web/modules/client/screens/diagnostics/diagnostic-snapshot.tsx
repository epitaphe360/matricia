import { formatExactScore, type DiagnosticDetail, type Opportunity } from "@/modules/shared/lib/diagnostics-opportunities/model";
...
export function DiagnosticSnapshot({ detail, locale, m }: { detail: DiagnosticDetail; locale: Locale; m: Messages }) {
  ...
  return <section aria-labelledby="diagnostic-snapshot" className="rounded-xl border bg-card p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 id="diagnostic-snapshot" className="text-xl font-semibold">{m.snapshot}</h2><p className="mt-1 text-sm text-muted-foreground">{m.snapshotIntro}</p></div><span className="rounded-full border px-3 py-1 text-sm">{detail.status === "SUPERSEDED" ? m.superseded : m.current}</span></div><dl className="mt-4 grid min-w-0 gap-3 text-sm sm:grid-cols-2"><Datum label={m.policyVersion} value={policyVersion}/><Datum label={m.formula} value={formula}/><Datum label={m.completedAt} value={new Intl.DateTimeFormat(locale === "ar" ? "ar-MA" : "fr-MA", { dateStyle: "medium", timeStyle: "short" }).format(new Date(detail.completedAt))}/></dl><details className="mt-4 rounded-lg border p-3 text-sm"><summary className="min-h-11 cursor-pointer py-2 font-medium">{m.technicalDetails}</summary><dl className="mt-3 grid min-w-0 gap-3 sm:grid-cols-2"><Datum label={m.session} value={detail.sessionId} mono/><Datum label={m.library} value={detail.libraryId} mono/><Datum label={m.inputManifest} value={detail.inputManifestHash} mono/><Datum label={m.policyHash} value={policyHash} mono/><Datum label={m.questionnaireVersion} value={questionnaireVersion} mono/></dl></details></section>;
}

export function SubscoreExplanation({ subscore, m }: { subscore: DiagnosticDetail["subscores"][number]; m: Messages }) {
  return <div><strong dir="ltr">{formatExactScore(subscore.score)}/100</strong><p className="text-sm text-muted-foreground">{formatDimensionKey(subscore.key)}</p><dl className="mt-2 grid gap-2 text-sm"><Datum label={m.weightedPoints} value={subscore.weightedPoints}/><Datum label={m.maximumPoints} value={subscore.maximumPoints}/><Datum label={m.explanation} value={summarize(subscore.explanation)}/></dl></div>;
}
