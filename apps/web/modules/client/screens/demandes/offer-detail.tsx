...
function PointIcon({ id }: { id: ClientOfferPoint["id"] }) {
  ...
  if (id === "exclusions") return <Info className="size-4" aria-hidden />;
...
function CheckList({ title, items, empty }: { title: string; items: readonly string[]; empty: string }) {
  return (
    <div className="client-offer-block">
      <h3>{title}</h3>
      <ul>
  ...
  return (
    <main className="client-page">
      ...
      <header className="client-mast client-offer-mast">
        <div>
          <h1>{copy.title} <em>{detail.label}</em></h1>
          <p>{copy.lead}</p>
        </div>
        <div className="client-offer-actions">
          <Link href={detail.comparisonHref} className="client-ghost-link"><BackIcon className="size-4" aria-hidden />{copy.backCompare}</Link>
          <Link href={detail.askHref} className="client-soft-link"><MessageSquare className="size-4" aria-hidden />{copy.askQuestion}</Link>
        </div>
      </header>

      <section className="client-board client-board-compare">
        <article className="client-card" aria-labelledby="cover-title">
          <header><h2 id="cover-title"><FileText className="size-4" aria-hidden />{copy.coverTitle}</h2></header>
          <div className="client-offer-cover">
            <CheckList title={copy.scope} items={detail.inclusions} empty={copy.emptyList} />
            <CheckList title={copy.deliverables} items={detail.deliverables} empty={copy.emptyList} />
            <CheckList title={copy.delays} items={detail.delays} empty={copy.emptyList} />
            <CheckList title={copy.conditions} items={detail.conditions} empty={copy.emptyList} />
          </div>
        </article>
        <article className="client-card" aria-labelledby="summary-title">
          <header>
            <h2 id="summary-title"><Scale className="size-4" aria-hidden />{copy.summaryTitle}</h2>
          </header>
          <p>{copy.summaryLead}</p>
          <ul className="client-offer-points">
            {detail.points.map((point) => (
              <li key={point.id}>
                <span className="client-feed-icon" data-tone={point.action === "review" ? "sky" : "peach"}><PointIcon id={point.id} /></span>
                <span>
                  <strong>{offerPointTitle(point.id, locale)}</strong>
                  <small>{point.hint}</small>
                </span>
                <em>{point.action === "review" ? copy.review : copy.compare}</em>
              </li>
            ))}
          </ul>
        </article>
      ...
      <section className="client-board client-board-compare">
        <article className="client-card client-compare" aria-labelledby="quote-title">
          <header><h2 id="quote-title"><FileSpreadsheet className="size-4" aria-hidden />{copy.quoteTitle}</h2></header>
          {detail.lines.length === 0 ? <p>{copy.emptyList}</p> : (
        ...
        <article className="client-card" aria-labelledby="docs-title">
          <header><h2 id="docs-title"><Paperclip className="size-4" aria-hidden />{copy.documentsTitle}</h2></header>
          <p>{copy.documentsLead}</p>
          {detail.documents.length === 0 ? <p>{copy.noDocuments}</p> : (
      ...
      <section className="client-board client-board-compare">
        <article className="client-card" aria-labelledby="flags-title">
          <header><h2 id="flags-title">{copy.flagsTitle}</h2></header>
          {detail.flags.length === 0 ? <p>{copy.noFlags}</p> : (
            <ul className="client-offer-flags">
              {detail.flags.map((flag) => (
                <li key={flag.id}>
                  <span className="client-feed-icon" data-tone={flag.tone === "clarify" ? "peach" : "sky"}><Info className="size-4" aria-hidden /></span>
                  <span>{flag.text}</span>
                  <em>{flag.tone === "clarify" ? copy.clarify : copy.confirm}</em>
                </li>
              ))}
            </ul>
          )}
        ...
        <article className="client-card client-offer-hint" aria-labelledby="hint-title">
          <h2 id="hint-title">{copy.compareHint}</h2>
          <p>{copy.compareHintBody}</p>
          <Link href={detail.comparisonHref} className="client-text-link">{copy.backCompare}<ArrowRight className="size-4 rtl:rotate-180" aria-hidden /></Link>
        </article>
      </section>
