...
import { formatBasisPointsExact, isTaxRuleApplicable, taxRulesForCategory, type ProviderQuoteDashboard } from "@/modules/provider/data/quotes/model";
...
function QuoteForm({ item, taxRules, organizationId, locale, m, identity, prefill }: { item:ProviderQuoteDashboard["invitations"][number];taxRules:ProviderQuoteDashboard["taxRules"];organizationId:string;locale:"fr"|"ar";m:ProviderQuoteMessages;identity:{revision:string;correlation:string};prefill?:QuotePrefill }) {
  ...
  return <form ref={formRef} action={action} onInput={recompute} onSubmit={saveLocal} className="grid min-w-0 gap-4 rounded-xl border p-4"><input type="hidden" name="locale" value={locale}/><input type="hidden" name="rfqProviderId" value={item.id}/><input type="hidden" name="currency" value={item.currency}/><input type="hidden" name="idempotencyKey" value={operationIdentity.revision}/><input type="hidden" name="correlationId" value={operationIdentity.correlation}/>
    ...
    <div className="flex flex-col gap-2 sm:flex-row"><Button type="button" variant="outline" disabled={pending} className="min-h-11" onClick={saveLocal}>{m.saveLocal}</Button><Button disabled={pending || !formValid || applicableRules.length===0} className="min-h-11">{pending?m.saving:m.save}</Button>{item.quote?.id?<Link href={`/${locale}/sous-traitant/devis/${item.quote.id}/apercu?organizationId=${organizationId}`} className="inline-flex min-h-11 items-center justify-center rounded-md border px-4 text-sm font-medium">{m.preview}</Link>:null}</div>
...
export function QuoteSubmitForm({ item, locale, m, identity }: { item:ProviderQuoteDashboard["invitations"][number];locale:"fr"|"ar";m:ProviderQuoteMessages;identity:{submit:string;correlation:string} }) {
