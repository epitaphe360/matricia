...
export async function createServerCreditsRepository(requestedOrganizationId?: string) {
  ...
  const scope = (ids: string[]) => requested && ids.includes(requested) ? [requested] : requested ? [] : ids.length === 1 ? ids : [];
  return createCreditsRepository({
    ...
    async benefits() { const now = new Date().toISOString(); return c.from("benefit_versions").select("id,version,benefit_type,fulfillment_mode,name_fr,name_ar,description_fr,description_ar,credit_cost,currency,effective_from,effective_to,library_id").eq("status", "ACTIVE").lte("effective_from", now).or(`effective_to.is.null,effective_to.gt.${now}`).limit(300); },
