...
| 17 — Observabilité/DLQ | P1 | PARTIAL | request/correlation IDs, latence/readiness, retry borné, dead-letter Outbox et vue Opérations en lecture seule (`20260910001000_outbox_dispatch_protocol.sql:4-93`; `packages/observability/src/logger.ts:19-20,102-103`; `apps/web/modules/admin/screens/operations/operations-dashboard.tsx:8-22`). Manquent reprocessing admin idempotent/four-eyes, métriques exhaustives webhooks/jobs et alerting opérationnel; l’écran déclare explicitement le replay absent (`operations/messages.ts:6`). |
