...
| MAT-FUNC-025 | Réception critère par critère : `accept_delivery`, `acceptance_checklists`, actions Client et panneaux missions. | SQL `0045`; `apps/web/modules/client/screens/missions/actions.test.ts`; route/a11y seulement dans `v1-critical-flows.spec.ts`. | `PARTIAL` — E2E de soumission, décision de chaque critère, rejet, correction puis acceptation complète; DENY d'un autre tenant et visa atomique manquent. |
